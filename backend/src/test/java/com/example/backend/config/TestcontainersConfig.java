...
public class TestcontainersConfig {
  ...
  public GenericContainer<?> authorizationServerContainer() {
    Path configPath = Paths.get("backend/src/test/resources").toAbsolutePath();
    GenericContainer<?> authorizationServer =
        new GenericContainer<>(DockerImageName.parse("ghcr.io/markoniemi/oauth2-server:latest"))
            ...
            .withFileSystemBind(configPath.toString(), "/app/config", BindMode.READ_ONLY)
            .withEnv("OAUTH2_CONFIG_PATH", "/app/config/oauth2-config.yaml")
